...
public class StockManager {
	...
	public StockManager() {
		...
		for (int i = 0; i <= db.getProductIDs().size() - 1; i++) {
			addProduct(Integer.toString(db.getProductIDs().get(i)), db
					.getProductNames().get(i), 0, "", "", 0, 0, 0, 0, "", db
					.getProductQuantities().get(i));
		}
	...
	public void addProduct(String productId, String productName,
			double productPrice, String imageLocation, String description,
			int length, int width, int height, int weight, String category,
			int productQuantity) {
		product.add(new Product(productId, productName, productPrice,
				imageLocation, description, length, width, height, weight,
	...
	public void insertNewRow(String productName, double productPrice,
			String imageLocation, String description, int length, int width,
			int height, int weight, String category, int productQuantity) {
		db.insertToDb(productName, productPrice, imageLocation, description,
				length, width, height, weight, category, productQuantity);
	}
	...
	public void saveReportToFile(File saveLoc) {
		try {
			...
			for (int i = 0; i <= product.size() - 1; i++) {
				...
				} else if (product.get(i).getProductQuantity() > 99
						&& product.get(i).getProductQuantity() < 1000) {
					report += product.get(i).getProductQuantity() + "     |";
				} else if (product.get(i).getProductQuantity() > 999
						&& product.get(i).getProductQuantity() < 10000) {
					report += product.get(i).getProductQuantity() + "     |";
				}
