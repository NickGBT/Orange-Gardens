...
public class OrderPrioritySystem {
	...
	private ArrayList<Order> placedOrders = (ArrayList<Order>) orders
			.findByStatus(OrderStatus.placed);
	...
	public List<Order> getPrioritisedQueue() {
		...
		for (Order o : placedOrders) {
			if ((currentTime - o.getDatePlacedInMillis()) < (360000 * 6)
					|| (currentTime - o.getDatePlacedInMillis()) > (360000)) {
		...
		for (Order o : placedOrders) {
			if ((currentTime - o.getDatePlacedInMillis()) < (360000 * 24)
					|| (currentTime - o.getDatePlacedInMillis()) > (360000 * 6)) {
				highPriority.add(o);
			}
		}
